...
fn find_ids<'a>(schema: &'a Value, id_mapping: &mut HashMap<String, &'a Value>, base_url: &url::Url) -> Result<(), ValidationError>{
    match schema {
        Value::Object(object) => {
...
impl<'a> Resolver<'a> {
    ...
    pub fn join_url(&self, url_ref: &str, stack: &validators::ScopeStack) -> Result<url::Url, ValidationError> {
    ...
    pub fn resolve_url(&self, url: &url::Url, instance: &'a Value) -> Result<&'a Value, ValidationError> {
        let url_str = url.as_str();
        match url_str {
            "document:///" => Ok(instance),
            "http://json-schema.org/draft-06/schema" => Ok(schemas::get_draft6_schema()),
    ...
    pub fn resolve_fragment(&self, url: &str, stack: &validators::ScopeStack, instance: &'a Value) -> Result<(url::Url, &'a Value), ValidationError> {
        ...
        let fragment = url::percent_encoding::percent_decode(
              url.fragment().unwrap_or_else(|| "").as_bytes()).decode_utf8().unwrap();
        // TODO Prevent infinite reference recursion
        match document.pointer(&fragment) {
            Some(x) => Ok((resource, x)),
            None => Err(
                ValidationError::new("Couldn't resolve JSON pointer")
            )
        }
    }
}
